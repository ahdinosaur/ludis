...
//! ## Drift precision
//!
//! SSH recipients are identified by a deterministic 4-byte tag in the stanza
//! header derived from the pubkey, so every drift case (add, remove, swap)
//! is caught precisely.
...
pub(crate) enum DriftReason {
    /// Header SSH stanza counts don't match: expected `{expected}`, found `{actual}`.
...
///
/// SSH stanzas carry a deterministic 4-byte `args[0]` tag derived from the
/// recipient's pubkey, so every drift case (add, remove, swap) is caught
/// precisely. Returns `None` when the header matches the resolved recipients;
/// `Some(reason)` describes the first mismatch found.
pub(crate) fn compare_stanzas(
    ...
    if expected_ssh_tags.len() != actual_ssh_tags.len() {
        return Some(DriftReason::CountMismatch {
            expected: expected_ssh_tags.len(),
            actual: actual_ssh_tags.len(),
        });
    }
...
/// recipient in a ciphertext header.
///
/// The tag is the first arg of an `ssh-ed25519` / `ssh-rsa` stanza - derived
/// from the SHA-256 of the SSH wire-format pubkey. We round-trip through a
/// one-byte encrypt to extract it without depending on `age`'s internals.
fn ssh_stanza_tag(key: &crate::key::Key) -> Option<String> {
    let recipient: Box<dyn age::Recipient + Send> = Box::new(key.recipient().clone());
...
impl std::fmt::Display for DriftReason {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            DriftReason::CountMismatch { expected, actual } => {
                write!(f, "expected {expected} SSH recipients, found {actual}")
            }
