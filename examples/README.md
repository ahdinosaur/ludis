...
| [`podman-compose/`](./podman-compose/) | A two-service podman-compose project (nginx + redis) managed via `@resource/podman state: "compose_present"`. Shows compose lifecycle and hash-based drift detection. | any Linux with `podman-compose` |
