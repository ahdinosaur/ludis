...
- Both resources accept an opt-in `sudo: true` flag (default `false`). When set, the write/symlink/remove and any follow-up chmod/chown shell out under `sudo -n` so a `local apply` can land target paths under `/etc/`, `/var/`, etc. without an external stage-and-install workaround. State probes also run under `sudo -n` when the flag is set (via `resource/src/probe.rs`), so paths whose parent the operator can't traverse (e.g. `/etc/sudoers.d/` at `0750 root:root`) are still observable. See `operation/src/sudo.rs` for the shared apply-side shell-out helpers and `resource/src/probe.rs` for the probe-side equivalents.
